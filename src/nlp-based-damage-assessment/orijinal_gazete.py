        ...
        if url['url'] in self.local_news_cache:
            ...
            try:
                ...
                self.news.append({
                    ...
                    'url': article.url,
                    'meta_url': article.meta_data.get('url'),
                    'meta_img': article.meta_img,
                    'meta_published_date': article.meta_data.get('datePublished'),
...
if __name__ == "__main__":
    scraper = NewsScraper(page_limit=3)
    keywords = [
    "deprem", "yıkım", "enkaz", "arama kurtarma", 
    "çadır kent", "kalıcı konut", "hasar tespiti", 
    "AFAD", "felaket",    
    "Hatay",
    "Kahramanmaraş",
    "Gaziantep",
    ...
    df.to_csv("orijinal_code_gazete.csv", index=False, encoding="utf-8-sig")
    print(f"\n✅ Toplam {len(df)} haber 'orijinal_code_gazete.csv' dosyasına kaydedildi.")
